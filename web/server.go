...
func SetServerLogger(logger *log.Logger) func(*Server) {
	return func(s *Server) {
		s.Logger = logger
	}
}
...
func (s *Server) addRoute(r string, method string, handler interface{}) {
	...
	s.Logger.Printf("Regist http service handler=%s,method=%s\n", r, method)
}
...
func (s *Server) Run(addr string) {
	...
	s.Logger.Printf("http serving %s\n", addr)
...
func (s *Server) RunFcgi(addr string) {
	s.initServer()
	s.Logger.Printf("http serving fcgi %s\n", addr)
...
func (s *Server) RunScgi(addr string) {
	s.initServer()
	s.Logger.Printf("http serving scgi %s\n", addr)
...
func (s *Server) working(addr string) {
	defer func() {
		if !s.Closed() {
			...
			if e := recover(); e != nil {
				s.Logger.Printf("recover http error=%+v\n", e)
			}
		}
	}()
	mux := http.DefaultServeMux
...
func (s *Server) startListen(trytime int, addr string) error {
	...
	if err != nil {
		if trytime > 0 {
			...
			s.Logger.Printf("tcp: Listen error:%s; retrying %d\n", err, trytime+1)
...
func (s *Server) startListenTLS(trytime int, addr string, config *tls.Config) error {
	...
	if err != nil {
		if trytime > 0 {
			time.Sleep(1 * time.Second)
			s.Logger.Printf("tcp: Listen error:%s; retrying %v\n", err, trytime+1)
		...
		if e := recover(); e != nil {
			if !s.Config.RecoverPanic {
				...
				case *gerror.SysError:
					s.Logger.Printf("Handler crashed with content=%+v\n", e)
				default:
					s.Logger.Printf("Handler crashed with content=%+v\n", e)
				}
			}
...
func (s *Server) logRequest(ctx Context, sTime time.Time) {
	...
	if len(ctx.Params) > 0 {
		//ctx.Server.Logger.Println(fmt.Sprintf("%s %s %s - %v -Params: %+v", client, req.Method, requestPath, duration, ctx.Params))
		ctx.TracePrintf("%s %s %s - %v -Params: %+v", client, req.Method, requestPath, duration, ctx.Params)
	} else {
		//ctx.Server.Logger.Println(fmt.Sprintf("%s %s %s - %v", client, req.Method, requestPath, duration))
