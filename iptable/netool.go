...
func init() {
	...
	go func() {
		defer func() {
			log.Printf("DEFAULT LOCAL IP MASK:%s | %s\n", InterIPNet.String(), InterExternalIp.String())
		}()
		//初始化默认网关
...
func LoadIpTable() {
	...
	if options, err := cfg.SectionOptions(section); err == nil && options != nil {
		...
		log.Printf("LOAD IP TABLE FILTER:%+v\n", trustmap)
		//替换存在的
		TrustFilterMap = trustmap
	} else {
		log.Printf("LOAD IP TABLE FILTER ERROR:%v\n", err)
	}
}
