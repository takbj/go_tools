...
func TestRouting(t *testing.T) {
	for _, test := range tests {
		...
		if resp.statusCode != test.expectedStatus {
			t.Fatalf("%v(%v) expected status %d got %d,data=%+v", test.method, test.path, test.expectedStatus, resp.statusCode, resp)
		}
...
func TestReadScgiRequest(t *testing.T) {
	headers := map[string][]string{"User-Agent": {HTTP_HEAD}}
